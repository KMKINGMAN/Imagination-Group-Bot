    ...
    run: async(data, client)=> {
        ...
        awards.forEach(async(award) => {
            if(data.DB.points >= award.points && !award.users.includes(data.General.member.id)){
                await guild_data.add_user(Number(award.points), data.General.member.id);
                client.emit("kmcodesAwardClame", { role: award.role, member: data.General.member, Data: guild_data, points:  award.points})
            }
        })
    }
}